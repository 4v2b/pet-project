...
{
    private readonly IEdmModel _edmModel = ODataUtility.GetEdmModel();
    ...
    public async Task<DataPage<ProposalDto>> GetAllProposalsAsync(HttpRequest request)
    {
        ...
        var oDataQueryOptions = new ODataQueryOptions<Proposal>(oDataQueryContext, request);

        var proposals = await unitOfWork.Proposals.GetProposalsAsync(false, oDataQueryOptions);
        var pageItemsDto = mapper.Map<IEnumerable<ProposalDto>>(proposals.Items);
        var page = new DataPage<ProposalDto>(pageItemsDto, proposals.TotalCount);
